...
func RestartIOThread(instanceKey *InstanceKey) error {
	for _, cmd := range []string{`stop subordinate io_thread`, `start subordinate io_thread`} {
...
func ResetSubordinate(instanceKey *InstanceKey) (*Instance, error) {
	...
	if instance.ReplicationThreadsExist() && !instance.ReplicationThreadsStopped() {
		return instance, fmt.Errorf("Cannot reset subordinate on: %+v because replication threads are not stopped", instanceKey)
	}
	...
	if err != nil && strings.Contains(err.Error(), Error1201CouldnotInitializeMainInfoStructure) {
		log.Debugf("ResetSubordinate: got %+v", err)
		workaroundBug83713(instanceKey)
		_, err = ExecInstance(instanceKey, `reset subordinate /*!50603 all */`)
	}
	if err != nil {
